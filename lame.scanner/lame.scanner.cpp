...
public:
	...
	void ScanWithCallback(LPCSTR file)
	{
		if (!file) return;
		m_file_count++;
		lame_scan_file_with_callback(m_lame , file , &Lame_Scan_Callback , this);
		return;
	}
	void ShowEngineInfo()
	{
...
private:
	static  void __stdcall Lame_Scan_Callback(const char* fname , lame_scan_result* result , void* user_data)
	{
...
int main(int argc, char* argv[])
{
	...
	for (uint32_t i = 1 ; i < argc ; i++)
	{
		if (*argv[i] == '-')
		{
			...
			else if (stricmp(argv[i] , "-lame.v1") == 0){ bShowLicense = true;}
