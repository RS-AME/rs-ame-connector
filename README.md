...
- Unix + PowerPC- 国产操作系统（Linux家族） + 国产CPU（x86/MIPS）
...
- 关键内容检验技术
    针对 Windows PE 文件（包含.Net）,划定约15个关键内容区域，对这些区域按照特定算法计算指纹，并匹配恶意指纹库。该技术支持机器自动化处理，无需人工提取，效率高。
- 数字签名校验及匹配技术
...
7. 丰富的 文件格式识别能力
    ...
    - INF   ：  系统自动化脚本
    - MSCFB   ：  Microsoft Compound File Binary (CFB) file format 
    - ODF   ：  OpenOffice文件
